...
function getDayName(date) {
  const isoDate = date.replace('UTC', 'Z');
...
function getNextFriday(date) {
  ...
  const diff = current === 5 ? 7 : (5 - current + 7) % 7;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.trunc(diff / perday) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const isAfterStartDate = currentDate >= periodStartDate;
  const isBeforeEndDate = currentDate <= periodEndDate;
  const isInPeriod = isAfterStartDate && isBeforeEndDate;
  return isInPeriod;
}
